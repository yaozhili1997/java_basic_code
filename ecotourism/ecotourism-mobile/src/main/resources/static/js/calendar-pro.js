(function(){
	...
	function getIndexDay(){
		...
		for( var k = 0;k<calendarDate.days;k++ ){
			...
			for( var d in calendarDate.opt.data ){
				...
				if (!selectedDate) {
					selectedDate = calendarDate.opt.data[0].date;
				}
				var isSelected = checkDate(selectedDate, dataDate);
				if( flag ){
					riliHtml += '<div class="ht-rili-td ht-rili-onclick ' + (isSelected ?  'ht-rili-td-active' : '') + '" data-date="'+ dataDateStr +'"><span class="ht-rili-day">'+ (k + 1) +'</span><span class="ht-rili-money" data-money="'+ calendarDate.opt.data[d].data +'">&yen;'+ calendarDate.opt.data[d].data +'</span></div>';
					break;
				}
			}
			if( !flag ){
	...
	function dateClick(obj){
		...
		var data = $(obj).parent().calendarGetActive();
		selectedDate = data.date;
		var callback = calendarDate.opt.callback;
    ...
    $.fn.extend({
        calendar:function(opt){
            ...
            if( (opt.beginDate != undefined && opt.endDate != undefined )||( opt.data.length > 0 ) ){
                ...
                var beginDate = opt.data[0].date;//设置日历开始日期
                console.log(beginDate);
                var endDate = opt.data[opt.data.length-1].date;//设置日历结束日期
                console.log(endDate);
				...
				if (opt.selectedDate) {
                    calendarDate.year = parseInt(opt.selectedDate.split('-')[0]);//设置起始日期为当前日期
                    calendarDate.month = parseInt(opt.selectedDate.split('-')[1]);//设置起始日期为当前日期
                    calendarDate.date = parseInt(opt.selectedDate.split('-')[2]);//设置起始日期为当前日期
				} else {
            ...
            //加载容器
            calendarDate.container = '<div class="calendar-mask" style="position: absolute;top:0;bottom: 0;left:0;right:0;background-color: rgba(0, 0, 0, .3);z-index: 4000;"><div style="position: absolute;bottom: 0;left:0;bottom: 0;"><div class="ht-rili-querybox"><strong class="ht-rili-title">'+ opt.title +'</strong><div class="ht-rili-datebox"><span class="ht-rili-leftarr mui-icon mui-icon-arrowleft"></span><span class="ht-rili-date"></span><span class="ht-rili-rightarr mui-icon mui-icon-arrowright"></span></div></div><div class="ht-rili-head"><div class="ht-rili-th">周日</div><div class="ht-rili-th">周一</div><div class="ht-rili-th">周二</div><div class="ht-rili-th">周三</div><div class="ht-rili-th">周四</div><div class="ht-rili-th">周五</div><div class="ht-rili-th">周六</div></div><div class="ht-rili-body"><!--<div class="ht-rili-td"><span class="ht-rili-day">1</span><span class="ht-rili-money">&yen;100</span></div>--></div><div><button type="button" class="mui-btn mui-btn-danger mask-close" style="width: 100%;height: 40px;border-radius: 0px;">关闭</button></div></div></div>';
            $(this).html(calendarDate.container);
