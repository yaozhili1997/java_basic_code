 ...
 * under the License.
 */
package com.ecotourism.oms.oms.util.express.util;

import com.ecotourism.oms.common.utils.MD5;
import com.ecotourism.oms.oms.util.express.constant.Constants;
import com.ecotourism.oms.oms.util.express.constant.HttpHeader;
import com.ecotourism.oms.oms.util.express.constant.SystemHeader;
...
public class SignUtil {
	...
	{	
		try {
				Mac hmacSha256 = Mac.getInstance(Constants.HMAC_SHA256);
				byte[] keyBytes = (cid+appKey+secret+jsonData).getBytes(Constants.ENCODING);
				hmacSha256.init(new SecretKeySpec(keyBytes, 0, keyBytes.length,
						Constants.HMAC_SHA256));
    ...
    private static String buildHeaders(Map<String, String> headers, List<String> signHeaderPrefixList) {
    	...
    	if (null != signHeaderPrefixList) {
    		...
    		if (null != headers) {
    			...
    			for (Entry<String, String> header : sortMap.entrySet()) {
